...
function pkgExp(c){if(!c.pstart)return null;var d=new Date(c.pstart);d.setDate(d.getDate()+30);return d.toISOString().split('T')[0];}
...
function pgPaketi(){
  var cards=pkgs.length?pkgs.map(function(p){
    return '<div class="pkc">'+
      ...
      '</div>'+
      '<div style="margin-bottom:6px"><span class="tchip '+TC[p.t]+'">'+TL[p.t]+'</span></div>'+
      '<div class="pkn">'+p.s+'</div>'+
      '<div class="pks">'+t('sessions')+' / 30 dana</div>'+
  ...
  return '<div class="topbar"><div class="ptitle">'+t('packages')+'</div><button class="btn btnp" onclick="openPkgM()">+ Novi paket</button></div>'+
    ...
    '<div style="font-size:12px;color:var(--text3)">Broj treninga unosiš slobodno. Svaki paket traje 30 dana.</div>';
}
function openPkgM(id){
  ePid=id||null;pkgTy='p';
  document.getElementById('mPkgt').textContent=id?t('edit')+' paket':'Novi paket';
  if(id){var p=pgb(id);document.getElementById('pses').value=p.s;document.getElementById('pprc').value=p.p;document.getElementById('pnam').value=p.n||'';pkgTy=p.t||'p';}
  else{document.getElementById('pses').value='';document.getElementById('pprc').value='';document.getElementById('pnam').value='';}
...
function savePkg(){
  ...
  if(ePid){var i=pkgs.findIndex(function(p){return p.id===ePid;});pkgs[i]=Object.assign({},pkgs[i],{s:s,p:pr,n:n,t:pkgTy});}
  else pkgs.push({id:Date.now(),s:s,p:pr,n:n,t:pkgTy});
